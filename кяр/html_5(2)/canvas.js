...
A.moveTo(74,400);
A.lineTo(550,400); //горизонтальная линия графика

A.moveTo(80,406);
A.lineTo(80,45); //вертикальная линия графика
//A.shadowOffsetX=6;
//A.shadowColor='grey';
A.stroke();//для завершения рисования линии
...
var x = 0;
for (i = 405; i >= 60; i-=30) {
    A.fillText(x, 53, i); //("текст",x,y)
    x++;
}



var grad =A.createLinearGradient(30,200,40,300);
grad.addColorStop(0.0,'PaleTurquoise'); //Значение от 0.0 до 1.0, которое представляет позицию между началом и концом градиента
grad.addColorStop(1.0,'Cornsilk');
A.fillStyle=grad;
...
A.fillRect(130, 281, 60, 118); //(x,y,ширина,высота) 
...
A.fillStyle = grad//'rgb(160, 209, 238)';
A.fillRect(460, 0, 60, 399);

A.beginPath();
A.moveTo(490, 390);
A.lineTo(490, 20); //стрелка
...
B.lineWidth = 8;
B.arc(200, 270, 135, 0, 2*Math.PI); //(x,y,радиус,нач_угол,конеч_угол)
B.stroke();

B.beginPath();
B.fillStyle = "rgba(253, 226, 62, 0.65)";
B.arc(200, 270, 132, 0, Math.PI/180*105, false); //По умолчанию установлено значение false, что означает по часовой стрелке.
B.lineTo(200, 270);
B.fill();
